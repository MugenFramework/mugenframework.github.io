...
| `UserAgent` | string | Agent User-Agent (Tengu: embedded in payload; Demon: used as filter) |
| `Uris` | list | URI list (Tengu: agent rotates through them per beacon; Demon: used as filter) |
| `Headers` | list | Tengu only: HTTP request headers embedded in payload and sent on every check-in |
| `Response.Headers` | list | Extra headers added to server HTTP responses |
...
### Tengu HTTP payload fields

Three listener fields are embedded in the Tengu binary at build time and control how the agent behaves at runtime. Rebuild the payload after changing them.

| Field | Effect |
|---|---|
| `UserAgent` | Overrides the default `Mozilla/5.0` string sent on every request |
| `Uris` | Agent cycles through the list per beacon (round-robin). Falls back to `/` if empty |
| `Headers` | Each string is appended as an HTTP request header on every check-in. Format: `"Name: Value"` |

:::note
`Headers` on a Tengu listener are **request** headers sent by the agent. For Demon, the same field controls server **response** headers - the semantics differ between agent types.
:::
