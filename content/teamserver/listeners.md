...
| User-Agent | Embedded in Tengu payloads at build time; used as a filter for Demon check-ins |
| URIs | Embedded in Tengu payloads - agent cycles through them per beacon (round-robin). Also used as the URI filter for Demon. |
| Headers | Custom HTTP request headers embedded in Tengu payloads and sent on every check-in. Format: `"Name: Value"`. |
